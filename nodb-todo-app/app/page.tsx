...
async function Home() {
  ...
  const todos = data.map((obj, index) => {
    return (
      <article key={obj.id} className={`${index === 0 ? 'rounded-t' : data.length - 1 !== index ? '' : 'rounded-b'} grid grid-cols-4 auto-rows-auto p-6 hover:bg-slate-200 hover:cursor-pointer`}>
